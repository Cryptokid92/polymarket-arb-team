...
- `stats.json` — listed / universe / gap / intent / reject / fill counts, `bankroll`, `daily_pnl`, closest edge, histogram, list window, `list_wraps`, unique listed/walked, `heartbeat_ms`
- `seen_markets.json` — unique condition ids listed or walked across windows (gitignored)
- `list_cursor.json` — official `next_cursor` plus next window number and wrap count. `None` cursor means the next list is catalog page 1.
